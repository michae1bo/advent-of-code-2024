...
export async function dayEightA(): Promise<number> {
  ...
  const towerMap: string[][] = rows.map((row: string): string[] =>
    row.split("")
  );
  const antinodeMap: string[][] = rows.map((row: string): string[] =>
    row.split("")
  );
  for (let i = 0; i < towerMap.length; i++) {
    for (let j = 0; j < towerMap[0].length; j++) {
      ...
      getAntinodes(towerMap, i, j).forEach((coordinates: number[]): void => {
        antinodeMap[coordinates[0]][coordinates[1]] = "#";
      });
    }
  }
  antinodeMap.forEach((row: string[]): void => {
    numberOfAntinodes += row.filter((z: string): boolean => z === "#").length;
  });
...
export async function dayEightB(): Promise<number> {
  ...
  for (let i = 0; i < towerMap.length; i++) {
    for (let j = 0; j < towerMap[0].length; j++) {
      ...
      getAntinodesB(towerMap, i, j).forEach((coordinates: number[]): void => {
        antinodeMap[coordinates[0]][coordinates[1]] = "#";
      });
    }
  }
    ...
    for (let y = 0; y < map[0].length; y++) {
      if (map[x][y] === map[i][j] && (x !== i || y !== j)) {
        ...
        if (isInBound(map, x + xDistance, y + yDistance)) {
          if (
            !antinodeCoordinates.includes([x + xDistance, y + yDistance]) &&
            !(x + xDistance === i && y + yDistance === j)
          ) {
