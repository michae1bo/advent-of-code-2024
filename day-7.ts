...
export async function daySevenA(): Promise<number> {
  ...
  textRows.forEach((row: string) => {
    const numbers: number[] = row.split(/(?:\s)/).map((
      numberString: string,
...
export async function daySevenB(): Promise<number> {
  ...
  textRows.forEach((row: string) => {
    ...
    if (
      performOperationB("*", numbers) || performOperationB("+", numbers) ||
      performOperationB("||", numbers)
    ) {
      totalCalibrationResult += numbers[0];
    }
  });
...
function performOperation(operator: string, numbers: number[]): boolean {
  if (numbers.length === 2) {
    ...
    return performOperation(
      "*",
      [numbers[0], result].concat(numbers.slice(3)),
    ) || performOperation("+", [numbers[0], result].concat(numbers.slice(3)));
  }
}
...
function performOperationB(operator: string, numbers: number[]): boolean {
  if (numbers.length === 2) {
    ...
    return performOperationB(
      "*",
      [numbers[0], result].concat(numbers.slice(3)),
    ) ||
      performOperationB("+", [numbers[0], result].concat(numbers.slice(3))) ||
      performOperationB("||", [numbers[0], result].concat(numbers.slice(3)));
  }
}
