...
export async function daySixA(): Promise<number> {
    ...
    while (inBound) {
        ...
        if ((guard.position[0] + speedX >= mapArray.length || guard.position[0] + speedX < 0) || (guard.position[1] + speedY >= mapArray[guard.position[0]].length || guard.position[1] + speedY < 0)) {
            mapArray[guard.position[0]][guard.position[1]] = "X"
            numberOfDistinctPositions++;
            inBound = false;
        } else if (mapArray[guard.position[0] + speedX][guard.position[1] + speedY] === "#") {
...
export async function daySixB(): Promise<number> {
    ...
    for (let i = 0; i < mapArray.length; i++) {
        for (let j = 0; j < mapArray[0].length; j++) {
            ...
            currentSetps = 0; 
            if ((i !== guard.position[0] || j !== guard.position[1]) && mapArray[i][j] !== "#") {
                mapArray[i][j] = "O";
            } else {
                continue;
            }
            while (inBound) {
                ...
                } else if (mapArray[guard.position[0] + speedX][guard.position[1] + speedY] === "O") {
                    if (guard.direction < 3) {
                        guard.direction++;
                    } else {
                        guard.direction = 0;
