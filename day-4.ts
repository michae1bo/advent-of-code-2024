...
function howManyXMAS(letters:string[][], i:number, j:number, direction:number): number {
    function checkProgress(letters: string[][], x: number, y: number, progress: number): number {
...
function isXShapeMas(letters: string[][], i:number, j:number):boolean {
    ...
    if ((stringUp === goal || stringUp === goalReverse) && (stringDown === goal || stringDown === goalReverse)) {
        isXShape = true;
    }
